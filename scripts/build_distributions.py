...
def build_custom(root: Path, cfg: dict, build_root: Path, version: str) -> Path:
    ...
    cap_text = render_template(cap_tpl, {
        "CAPABILITY_RECOMMENDATIONS": (
            "- Webbsökning: aktivera för aktuella produkt-, versions- och standardfrågor\n"
            "- Dataanalys: behöver inte aktiveras för kärnfunktionen\n"
            "- Bildgenerering: behöver inte aktiveras\n"
            "- Filhantering: aktivera för uppladdade problemunderlag och ADR-leveranser"
        )
    })
    ...
    compat = render_template(compat_tpl, {
        "GPT_NAME": cfg["project"]["name"],
        "RUNTIME_RECOMMENDATION": "Båda distributionerna bär hela kärnflödet och samma mönsterkunskap. Välj efter önskat arbetssätt och tillgängliga plattformsverktyg.",
        "PARITY_TABLE": (
            "| Capability | Chat ZIP | Custom GPT |\n"
            "| --- | --- | --- |\n"
            "| Behovs- och drivkraftsanalys | equivalent | equivalent |\n"
            "| Mönsterrekommendation och avvägning | equivalent | equivalent |\n"
            "| Full Knowledge-katalog | equivalent | equivalent |\n"
            "| ADR-underlag och strukturerad JSON | equivalent | equivalent |\n"
            "| Aktuell produktresearch | platform_dependent | platform_dependent |"
        ),
        "REDUCED_FEATURES": "Webbsökning och filhantering beror på vilka verktyg som är aktiverade i den aktuella Chat- eller Custom GPT-miljön.",
        "MISSING_FEATURES": "Inga saknade kritiska capabilities i den byggda distributionen.",
    })
...
def build_claude(root: Path, cfg: dict, build_root: Path, version: str) -> Path:
    ...
    (out / "README.md").write_text(
        ...
        "Lägg project/instructions.md som Project Instructions och innehållet i project/knowledge/ som Project Knowledge/Files. "
        "Paketet är avsett för Claude Projects och använder inte CLAUDE.md eller andra Claude Code-specifika konventioner.\n",
        encoding="utf-8",
    )
    (out / "VERSION").write_text(version + "\n", encoding="utf-8")
