...
def validate(root: Path) -> None:
    ...
    for path in files:
        ...
        missing_fields = {"id", "title", "criticality", "input", "expected"} - set(case)
        if missing_fields:
            raise SystemExit(f"Invalid eval {path.name}; missing: {sorted(missing_fields)}")
        if case["criticality"] not in {"critical", "important", "optional"}:
            raise SystemExit(f"Invalid criticality in {path.name}")
        ids.add(case["id"])
